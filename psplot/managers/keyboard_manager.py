...
class KeyboardManager:
    ...
    def keyPressEvent(self, event):
        ...
        if Qt.Key_1 <= key <= Qt.Key_6:
            ...
            if current_page:
                # [BUGFIX] Disable changes while measurement cursors (T) are active
                if current_page.plot_canvas and getattr(current_page.plot_canvas, 'cursors_active', False):
                    # Show a brief hint in status for user feedback
                    self.main_window.status_help_label.setText("Subplot count change disabled while Cursors (T) are active!")
                    event.accept()
                    return
